...
export default function DashboardModule() {
  ...
  const fetchDashboard = async()=>{
    try{
      ...
      const [basicRes, advancedRes] = await Promise.all([
        api.get(`/analytics?storeId=${selectedStore}`),
        api.get(`/analytics/advanced?period=30&storeId=${selectedStore}`)
      ]);
  ...
  const exportToExcel = ()=>{
    const exportData=[
      ...
      ...(advancedData?.salesTrend || []).map(item=>[
        item.date,
        item.revenue,
        item.count
      ])
  ...
  const notificationActions = {
    LOW_STOCK:{ label:"View Inventory", path:"/admin/products" },
    INVENTORY:{ label:"View Inventory", path:"/admin/inventory" },
    SALE:{ label:"View Sales", path:"/admin/sales" },
    TARGET:{ label:"View Analytics", path:"/admin/analytics" },
    FAILED_LOGIN:{ label:"Review Security", path:"/admin/settings/security" },
    LOGIN:{ label:"Okay", acknowledge:true },
    LOGOUT:{ label:"Okay", acknowledge:true },
    SYSTEM:{ label:"Okay", acknowledge:true }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500">Real-time business performance</p>
        </div>
        ...
        <div className="flex gap-3 items-center">
          ...
          <button
            ...
            className="bg-slate-900 text-white px-5 py-3 rounded-2xl flex gap-2 items-center"
          >
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <RevenueChart salesTrend={advancedData?.salesTrend} />
        <BranchPerformanceCard branches={advancedData?.branchPerformance || []} />
      </div>

      <CashierLeaderboard cashiers={advancedData?.cashierPerformance || []} />

      {/* NEW CARDS */}
      <div className="grid lg:grid-cols-2 gap-6">
        <TopProductsCard products={advancedData?.topProducts || []} />
        <FinanceSummaryCard finance={advancedData?.finance} />
      </div>

      <InventoryInsightsCard inventory={advancedData?.inventoryAnalytics} />

      {/* LOW STOCK */}
      <LowStockAlert products={data.lowStock} />
    </div>
  );
}
